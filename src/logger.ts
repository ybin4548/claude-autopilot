...
export const consoleLogger: Logger = {
  ...
  groupStart(groupIndex, totalGroups, taskCount) {
    ...
    console.log(`\n> Group ${groupIndex + 1}/${totalGroups} — ${taskCount} task${taskCount > 1 ? 's' : ''} ${parallel}`);
  },
  ...
  taskExecuted(taskId, success) {
    if (success) {
      ...
      console.log(`  [${taskId}] Execution failed`);
    }
  },
  ...
  taskRateLimited(taskId) {
    console.log(`  [${taskId}] Rate limited — waiting for recovery...`);
  },

  rateLimitRecovered() {
    console.log(`  Rate limit recovered — resuming`);
  },
